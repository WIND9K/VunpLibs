# vunpLibs v3.1 – Enterprise-grade REST API & Database Library

[![Version](https://img.shields.io/badge/version-0.3.1-blue.svg)](CHANGELOG.md)
[![Python](https://img.shields.io/badge/python-3.10+-blue.svg)](pyproject.toml)
[![License](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE)

vunpLibs là thư viện **REST-first** với **enterprise-grade database** để làm việc với hệ thống Cyclos/ONUS:
...
Mục tiêu: **App chỉ lo business**, còn hạ tầng (API/DB) do vunpLibs lo.

## 📦 Cài đặt

```bash
# Chỉ API
pip install vunpLibs

# API + Database
pip install vunpLibs[db]

# Full (bao gồm dev tools)
pip install vunpLibs[all,dev]
```
...
### API Usage
```python
from vunpLibs.unified.api import fetch_json
from vunpLibs.utils.date_utils import build_date_period
...
- HTTP & runtime:
  - `vunpLibs_BASE_URL` – bắt buộc (vd: `https://wallet.vndc.io`)
  - `vunpLibs_PAGE_SIZE`
  - `vunpLibs_REQ_PER_SEC`
  - `vunpLibs_MAX_INFLIGHT`
  - `vunpLibs_TIMEOUT_S`
  - `vunpLibs_HTTP2`
  - `vunpLibs_VERIFY_SSL`
  - `vunpLibs_PROXY`
  - `vunpLibs_PARALLEL` – phân trang song song theo page.

- **Hybrid auto-segment (v3):**
  - `vunpLibs_MAX_WINDOW_DAYS` – chia `datePeriod` lớn thành nhiều “cửa sổ ngày” (vd: 1 ngày / window).
  - `vunpLibs_MAX_ROWS_PER_WINDOW` – trần số record ước tính / window (dựa `X-Total-Count`).
  - `vunpLibs_AUTO_SEGMENT` – bật/tắt auto-segment (mặc định `true`).
  - `vunpLibs_MAX_SEGMENT_SPLIT_DEPTH` – số lần chia đôi tối đa khi fallback 422.
  - `vunpLibs_SEGMENT_PARALLEL`, `vunpLibs_SEGMENT_MAX_WORKERS` – chạy nhiều segment thời gian song song.

- Legacy:
  - `vunpLibs_DATE_SEGMENT_HOURS` – cấu hình segment theo giờ **cũ** (giữ lại cho code legacy, `fetch_json` v3 không dùng).

- Bảo mật & logging:
  - `vunpLibs_SECRETS_BACKEND` (`keyring`|`env`)
  - `vunpLibs_KEYRING_SERVICE`, `vunpLibs_KEYRING_ITEM`
  - `vunpLibs_FALLBACK_ENV`
  - `vunpLibs_TOKEN_HEADER`
  - `vunpLibs_LOG_LEVEL`, `vunpLibs_AUTO_DOTENV`, …
...
- Lấy token từ **Keyring**:
  - Service: `vunpLibs_KEYRING_SERVICE` (vd: `vunpLibs`)
  - Item: `vunpLibs_KEYRING_ITEM` (vd: `ACCESS_CLIENT_TOKEN`)
- Hoặc đọc token từ ENV nếu `vunpLibs_FALLBACK_ENV=true`.
- Trả về headers chuẩn:
  - `Access-Client-Token: <token>`
  - `User-Agent: vunpLibs/3 (Python x.y.z)`
...
Tất cả các request trong vunpLibs đi qua HttpClient.
...
```python
from vunpLibs.pagination.header_pager import HeaderPager
...
- `header_fetch_all_parallel`:
  ...
  - Giới hạn bởi `workers` hoặc `vunpLibs_MAX_INFLIGHT`.
...
1. Peek `X-Total-Count` 1 lần:
   - Gửi request với:
     - `datePeriod = window`,
     - `page=0`,
     - `pageSize = eff_page_size` (từ `page_size arg` hoặc `vunpLibs_PAGE_SIZE`).
...
1. **Page-level parallel**:
   - `vunpLibs_PARALLEL=true`.
   - Nhiều page trong cùng 1 window/segment được fetch song song.

2. **Segment-level parallel**:
   - `vunpLibs_SEGMENT_PARALLEL=true`.
   - `vunpLibs_SEGMENT_MAX_WORKERS` = số worker cho segment.
   - Nhiều segment (nhiều đoạn thời gian) chạy song song.

Cả 2 đều chịu giới hạn bởi `vunpLibs_MAX_INFLIGHT` và `vunpLibs_REQ_PER_SEC`.
...
```bash
# Connection settings
vunpLibs_DB_HOST=127.0.0.1
vunpLibs_DB_PORT=3306
vunpLibs_DB_USER=onusreport
vunpLibs_DB_PASSWORD=xxx
vunpLibs_DB_NAME=onusreport
vunpLibs_DB_CONNECT_TIMEOUT=10

# NEW v3.1: Connection pool settings
vunpLibs_DB_POOL_SIZE=5          # Số connections trong pool
vunpLibs_DB_MAX_OVERFLOW=10      # Số connections tối đa thêm
vunpLibs_DB_RETRY_COUNT=3        # Số lần retry
```
...
```python
from vunpLibs.db import DB, DbSettings
...
- **Parallel & segment:**
  ...
  - `vunpLibs_AUTO_SEGMENT`
  - `vunpLibs_MAX_SEGMENT_SPLIT_DEPTH`
  - `vunpLibs_SEGMENT_PARALLEL`
  - `vunpLibs_SEGMENT_MAX_WORKERS`
  - `vunpLibs_DATE_SEGMENT_HOURS` (legacy)

- **Bảo mật:**
  - `vunpLibs_SECRETS_BACKEND`
  - `vunpLibs_KEYRING_SERVICE`
  - `vunpLibs_KEYRING_ITEM`
  - `vunpLibs_FALLBACK_ENV`
...
- **Connection (Bắt buộc):**
  ...
  - `vunpLibs_DB_PORT`
  - `vunpLibs_DB_USER`
  - `vunpLibs_DB_PASSWORD`
  - `vunpLibs_DB_NAME`

- **Connection Pool (NEW v3.1 - Optional):**
  - `vunpLibs_DB_POOL_SIZE` (mặc định: 5)
  - `vunpLibs_DB_MAX_OVERFLOW` (mặc định: 10)
  - `vunpLibs_DB_RETRY_COUNT` (mặc định: 3)
  - `vunpLibs_DB_CONNECT_TIMEOUT` (mặc định: 10)
  - `vunpLibs_DB_SSL_CA`
